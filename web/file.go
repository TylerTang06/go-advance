...
func (f *FileUploader) Handle() HandleFunc {
	...
	return func(ctx *Context) {
		...
		if err != nil {
			...
			ctx.RespData = []byte("Failed to upload, not found data")
			log.Fatalln(err)
			return
		}
		defer src.Close()
		...
		if err != nil {
			...
			ctx.RespData = []byte("Failed to upload")
			log.Fatalln(err)
			return
		}
		defer dst.Close()
		...
		ctx.RespData = []byte("Upload successfully")
	}
}
...
func WithFileCache(maxFileSizeThreshold int, maxCacheFileCnt int) StaticResourceHandlerOption {
	return func(h *StaticResourceHandler) {
		...
		if err != nil {
			log.Printf("Failed to create cache, static resource not saved")
		}
...
func (h *StaticResourceHandler) Handle(ctx *Context) {
	...
	if item, ok := h.readFileFromData(req); ok {
		log.Printf("Reading data from cache...")
