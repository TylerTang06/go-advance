...
func (s *HTTPServer) serve(ctx *Context) {
	m, ok := s.findRoute(ctx.Req.Method, ctx.Req.URL.Path)
	if !ok || m.n == nil || m.n.handler == nil {
		ctx.RespStatusCode = 404
		return
	}
	ctx.PathParams = m.pathParams
	ctx.MatchedRoute = m.n.route
	m.n.handler(ctx)
}
...
func (s *HTTPServer) flashResp(ctx *Context) {
	...
	if err != nil {
		log.Fatalln("Failed to write back response", err)
	}
}
